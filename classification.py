...
def obtain_dataset(dir_path):
    if dir_path[-1] == '/':
        clone_featureCSV = dir_path + 'BCB_clone_euc_sim.csv'
        nonclone_featureCSV = dir_path + 'BCB_nonclone_euc_sim.csv'

    else:
        clone_featureCSV = dir_path + '/GCJ_clone_euc_sim.csv'
        nonclone_featureCSV = dir_path + '/GCJ_nonclone_euc_sim.csv'
...
def randomforest(vectors, labels):
    ...
    print('F1：{},P：{},R：{}'.format(np.mean(F1s), np.mean(Precisions), np.mean(Recalls)))
